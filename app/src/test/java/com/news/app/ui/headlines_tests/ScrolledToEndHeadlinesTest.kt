...
class ScrolledToEndHeadlinesTest: BehaviorSpec({
    ...
    Given("test scrolledToEnd function with no need to paginate") {
        ...
        every { viewState.hideLoading() } returns Unit
        When("calling scrolledToEnd") {
            headlinesPresenter.scrolledToEnd()
            Then("check if methods are not calling") {
                verify(exactly = 0) {
                    repository.getHeadlinesNews("general", 8, 2)
    ...
    Given("test scrolledToEnd function with need to paginate") {
        ...
        When("calling scrolledToEnd") {
            headlinesPresenter.scrolledToEnd()
            Then("check if methods are calling") {
                verify {
                    repository.getHeadlinesNews("general", 8, 2)
                    viewState.displayNewsList(arrayListOf())
                    viewState.hideLoading()
